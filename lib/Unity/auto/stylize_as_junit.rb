...
class UnityToJUnit
  ...
  def run
    ...
    results.each do |result_file|
      ...
      raise "Empty unittest result file: #{result_file}" if lines.empty?
  ...
  def parse_test_summary(summary)
    raise "Couldn't parse unittest results: #{summary}" unless summary.find { |v| v =~ /(\d+) Tests (\d+) Failures (\d+) Ignored/ }
  ...
  def write_failures(results, stream)
    ...
    result.each do |item|
      ...
      stream.puts "\t\t<testcase classname=\"#{@unit_name}\" name=\"#{item[:unittest]}\" time=\"0\">"
  ...
  def write_ignored(results, stream)
    ...
    result.each do |item|
      filename = File.join(results[:source][:path], File.basename(results[:source][:file], '.*'))
      puts "Writing ignored tests for unittest harness: #{filename}"
      stream.puts "\t\t<testcase classname=\"#{@unit_name}\" name=\"#{item[:unittest]}\" time=\"0\">"
...
if $0 == __FILE__
  ...
  begin
    ...
    targets = "#{options.results_dir.tr('\\', '/')}**/*.unittest*"
