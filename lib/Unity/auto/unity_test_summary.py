...
class UnityTestSummary:
    ...
    def run(self):
        ...
        for result_file in results:
            ...
            if len(lines) == 0:
                raise Exception("Empty unittest result file: %s" % result_file)
    ...
    def parse_test_summary(self, summary):
        ...
        if not m:
            raise Exception("Couldn't parse unittest results: %s" % summary)
...
if __name__ == '__main__':
  ...
  try:
    ...
    if len(sys.argv) > 1:
        ...
        targets_dir = './'
    targets = list(map(lambda x: x.replace('\\', '/'), glob(targets_dir + '**/*.unittest*', recursive=True)))
    if len(targets) == 0:
