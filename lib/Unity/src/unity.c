                            ...
                            const UNITY_LINE_TYPE lineNumber)
{
    ...
    { /* handle case of one pointers being null (if both null, unittest should pass) */
        if (expected != actual)
        {
            Unity.CurrentTestFailed = 1;
        }
...
/*-----------------------------------------------*/
/* If we have not defined our own unittest runner, then include our default unittest runner to make life easier */
...
{
    /* Check if this unittest name matches the included unittest pattern */
    int retval;
    ...
    /* Check if this unittest name matches the excluded unittest pattern */
