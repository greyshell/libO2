...
void tearDown(void)
{
    endPutcharSpy(); /* Stop suppressing unittest output */
...
{
    /* This unittest ensures that sizeof(struct UNITY_STORAGE_T) doesn't change. If this
     * unittest breaks, go look at the initialization of the Unity global variable
...
{
    TEST_PASS();
    TEST_FAIL_MESSAGE("We should have passed already and finished this unittest");
}
