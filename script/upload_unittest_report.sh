...
then
  ...
  curl -XPOST --data "payload={\"text\": \"> *Unittest* failed for \`${TEST_LIB}\` report: \
   $(cat test/debug/${TEST_LIB}/unittest_build/results/test_${TEST_LIB}.txt | pastebinit -b https://paste.ubuntu.com).\n Skipping \`Fuzz Test\`.\"}" $SLACK_WEBHOOK_URL
  exit $FAILED_CASES
fi
exit $FAILED_CASES